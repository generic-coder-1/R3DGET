...
use winit::{
    ...
    keyboard::{KeyCode, ModifiersState, PhysicalKey::Code},
    window::CursorIcon,
};
        ...
        match winit_event {
            Event::WindowEvent {
                ...
                ScaleFactorChanged { scale_factor, .. } => {
                    ...
                    self.raw_input.screen_rect = Some(egui::Rect::from_min_size(
                        Default::default(),
                        self.raw_input.screen_rect.unwrap().size() / self.scale_factor as f32,
                    ));
                }
                ...
                KeyboardInput { event, .. } => {
                    ...
                    if let Code(virtual_keycode) = input.physical_key {
                    ...
                    if let Some(ch) = &input.text {
                        if pressed && is_printable(ch.to_string().chars().next().unwrap()) {
                            self.raw_input
                                .events
